...
%TODO az introductionbe tisztázni a fogalmakat
...
Since the nominal noun tag is the most frequent amongst abbreviations, a plausible method (``UnkN'') is to assign the \textsc{n.nom} label to unknown ones. 
...
First, we utilize (``UnkUni'') a uniform distribution over their labels. % using the development set.
...
Our results (in Table  \ref{tab:eval_subcorpora}) on the development set indicate that training on a subcorpus cannot improve the performance. % , however the whole data yields a better tagger.
