...
Finally, domain adaptation enhancements are introduced which are evaluated on the test corpus.
...
Further on, medicines names are common as well, which found to be almost always singular nouns. 
Finally, numerous sentences lack final punctuation marks that are not recovered in the test corpus. 

The manually annotated corpus is split into two parts (cf. Table \ref{tab:clin_corpus}) for our experiments. 
The first one is employed for development purposes, while new methods are evaluated on the second part.
%
...
We use a baseline chain to analyze its errors and thus improve its annotation quality.
It is built on the Humor analyzer, which produces \emph{(morphosyntactic tag, lemma)} pairs as analyses. 
(The output of the \acrshort{ma} is extended with the new analysis of the \textit{x} token to fit the corpus to be tagged.)
Further on, analyses candidates are disambiguated by PurePos. 

This baseline tagger produced 86.61\% token accuracy\footnote{Precision is calculated considering correct full analyses of tokens, not counting punctuation marks.} on the development set, which is remarkably lower than tagging results for general Hungarian using the same components (96--98\% as in \cite{Orosz2013b,zsibrata2013magyarlanc}). 
Further on, sentence-based prevision shows that less than the third (28.33\%) of the sentences were tagged correctly. 
...
Another sorts of mistakes were caused by \acrlong{oov} words. 
These are specific to the clinical domain and often originate from Latin.
Although, this observation is in accordance with the \acrshort{pos} tagging results for medical English, listing of such terms' analyses is not a satisfactory solution to the problem, since the number of inflected forms are significantly larger compared to English. 
...
Based on the classification of errors above, domain-adaptation techniques are introduced enhancing the overall accuracy of the chain.
...
By employing the enhanced lexicon, the ratio of \acrshort{oov} words is reduced to 26.19\% (from 34.57\%) that also improved the overall accuracy to 92.41\% (on the development set). 
...
In doing so, the overall accuracy was increased notably, reducing the number of errors on the development set by 9.20\%.
...
Finally, the rest of the errors were connected to those abbreviations mainly which are both unknown to the analyzer and have not been seen previously. 
Therefore, the distribution of abbreviations' labels is compared to that of the Szeged Corpus (see Table \ref{tab:pos_distribution} below).
...
In doing so, we kept the original word forms as lemmata. 
...
Next, we tried to approximate the abbreviations' analyses with the distribution of tags observed in Table \ref{tab:pos_distribution}. 
First, we utilized (``UnkUni'') a uniform distribution over their labels. % using the development set.
In that way, \textsc{a.nom}, \textsc{a.pro}, \textsc{adv}, \textsc{conj}, \textsc{n.nom}, \textsc{v.3sg} and \textsc{v.pst\_ptcl} labels were used with equal probability as a sort of guessing algorithm.

Beside these, another reasonable method was to employ maximum likelihood estimation for calculating a priori probabilities of labels (``UnkMLE''). 
In that way, relative frequency estimates were calculated for all the tags enlisted above.
...
Since many studies showed (cf. Section \ref{sec:biomed_tag}) that the training data significantly affects the result of a data-driven annotation chain, we investigated sub-corpora of the Szeged Corpus. 
...
Besides, we compute perplexity, since it can measure similarities of texts \cite{kilgarriff1998measures}. 
...
Our examination shows that neither parts of the Szeged Corpus contain as much abbreviated terms as clinical texts have. 
...
The taggers' accuracy scores (cf. Table  \ref{tab:eval_subcorpora}) on the development set show that training on a subcorpus cannot improve the performance. 
Therefore, we decided to use utilize the whole Szeged Corpus to train our system.

\subsection{Evaluation}

The improved chain (cf. Table \ref{tab:improvements}) is evaluated by investigating the part-of-speech tagging, lemmatization and the whole morphological annotation performance.
...
However, the proper handling of abbreviations also increased the performance significantly.
...
We introduced numerous enhancements dealing with them, however not all of them were successful.
