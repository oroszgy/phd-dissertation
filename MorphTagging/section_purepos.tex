...
It contains general texts from six genres being annotated with detailed (MSD) morpho-syntactic tags~\cite{Erjavec2012} and lemmata. %TODO: ebben csak hírek vannak??
...
As regards lemmatization tools, CST~\cite{Jongejan} and a simple baseline method (BL) were employed. 
The latter one assigns the most frequent lemmata to a known \emph{(word, tag)} pairs, otherwise the root is considered be the word itself.
...
As regards \texttt{magyarlanc}, it also yields first-class results (95.72\% precision) on the MSD-tagged corpus, however, its language (and annotation scheme) specific components inhibit its application on other corpora. %TODO: morfológiai elemző, címke mappelés
