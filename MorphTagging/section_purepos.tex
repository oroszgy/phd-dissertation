...
At this point we simulated a setting, where the tagger is only load once, therefore relatively large lexicons were prepared for the tool.
...
On the other hand, the two corpora use different annotation schemes:
\begin{itemize}
...
\end{itemize}
Such differences can have a huge impact on morphological disambiguation algorithms.
...
As regards learning methods of the OpenNLP toolkit, their precision indicate that they cannot handle such labeling problems properly.
Further on, both of the lemmatization methods degrade accuracy, which can be due to their design. 
The baseline method was not prepared for handling unknown words, while CST was originally created for inflectional languages. 
...
Finally, results show that PurePos provides the most accurate annotations amongst tool tested.
In addition, sentence-based accuracies (especially on the Humor-tagged corpus) confirms its superior performance. 
They reveal that most of the tools result in erroneously tagged sentences in more than half of the cases, while the same number for our method is much less (42\% and 18\%). 
Therefore, it is shown that the presented algorithm can handle the huge number of inflected forms of Hungarian, while it can also produce precise root candidates for both previously seen and unseen tokens due to its improved lemma computing method.
...
Next, PurePos is compared to others (see methods investigated in the previous section\footnote{Unfortunately, we could not measure the performance of \texttt{magyarlanc}, since the current release of the tool cannot be trained.}), on settings when the amount of the training data is limited (cf. Table \ref{tab:small-szeged}).
In that way, we draw learning curves of systems evaluating them on both versions of the Hungarian Szeged Corpus.
...
First, Figures \ref{fig:msd-token} and \ref{fig:humor-token} present morphological tagging accuracies of the systems depending on the number of tokens in the training corpus. 
These results are in accordance with conclusions of our previous experiments, however the differences revealed are higher. 
Further on, the large distance between precision scores of PurePos and other systems confirms the effectiveness of our  hybrid approach in less-resourced scenarios.
...
Further on, if we compare (cf. Figures \ref{fig:msd-sent} and \ref{fig:humor-sent}) sentence precisions, the gap between taggers are much more emphasized. 
