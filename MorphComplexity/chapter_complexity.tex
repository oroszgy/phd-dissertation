...
Even though CLAN toolkit \cite{childesMit,macwhinney1991childes} can count morphemes in utterances, this feature is only available just for a few languages not containing any agglutinative ones. %TODO: Kinga szerint az MLUm nem biztos, hogy usually
...
In the case of corpora which follow the CHAT guidelines \cite{macwhinney1991childes,childesMit}, lengths of utterances (including morpheme counting) can be calculated \cite{sagae2010childes} with the CLAN \cite{MacWhinney1992} toolkit. 
This system is widely used, since it has components performing the necessary preprocessing steps. 
One of its modules is MOR \cite{childesMit} that is a morphological analyzer designed for spoken language corpora. 
